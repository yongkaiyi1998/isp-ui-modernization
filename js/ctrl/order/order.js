...
function upAction(id) {	
	...
	if(id != 1) {
		...
		for (counter = 0; counter < str.length; counter++) {
			...
			swap[this_id]		= $('#invoice_detail\\['+swapID+'\\]\\['+this_id+'\\]').val();
			current[this_id]	= $('#invoice_detail\\['+currentID+'\\]\\['+this_id+'\\]').val();
			
			//get set value to each selector		
			$('#invoice_detail\\['+swapID+'\\]\\['+this_id+'\\]').val(current[this_id]);
			$('#invoice_detail\\['+currentID+'\\]\\['+this_id+'\\]').val(swap[this_id]);			
		}		
		console.log("moved!");
	}else{		
		console.log("do nthg!");
...
function addAction() {		
	...
	var toFocus = '#invoice_detail\\['+i+'\\]\\[num\\]';
	$(toFocus).focus();	
...
function calGrandTotal() {
	...
	var gTotal = $('#'+db_table+'\\[grand_total\\]');
