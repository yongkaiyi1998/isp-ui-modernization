...
<script>
	...
	{
		var form = $('form');
		form.attr('action', base_url + 'report/export_terminated_acc');
		//form.attr('target', '_blank');
		$('#btSubmit').click();
		form.attr('action', base_url + 'report/terminated_acc') ;
...
$( document ).on("click", "#btPrint", function(e){
	var form = $('form');
	form.attr('action', base_url + 'report/terminated_acc/print');
	form.attr('target', '_blank');
	$('#btSubmit').val("print");
	$('#btSubmit').click();
	form.attr('action', base_url + 'report/terminated_acc');
...
function ajax_filter(order_by='', order_type='') {
	...
	$.ajax({
		type: "POST",
		url: base_url + "report/terminated_acc_rows/0",
		data: { 
...
function ajax_clear() {
		...
		<table class="table-striped" style="width:100%;">
			<thead>
				...
				<tr class="report_th" style="background: #289383; font-weight: bold; color: #fff;">
					...
					<th class="text-center" <?php if ($print == 1) { echo 'style="width:10%;"'; } ?>>Activated
					<?php if ($print != 1) { ?><i class="menu-icon fa fa-sort" id="activated_status_date"></i><?php } ?>
					</th>

					<th class="text-center" <?php if ($print == 1) { echo 'style="width:10%;"'; } ?>>Terminated
					<?php if ($print != 1) { ?><i class="menu-icon fa fa-sort" id="status_date"></i><?php } ?>
					</th>
					...
					<th class="text-center" <?php if ($print == 1) { echo 'style="width:10%;"'; } ?>>Last Pack.
	...
	ajax_clear_session('report', 'report_terminated_acc_filter');
}
