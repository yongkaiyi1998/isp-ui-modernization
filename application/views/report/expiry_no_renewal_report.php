...
<script>
	...
	{
		var form = $('form');
		form.attr('action', base_url + 'report/export_expiry_no_renewal_report');
		//form.attr('target', '_blank');
		$('#btSubmit').click();
		form.attr('action', base_url + 'report/expiry_no_renewal_report') ;
...
$( document ).on("click", "#btPrint", function(e){
	var form = $('form');
	form.attr('action', base_url + 'report/expiry_no_renewal_report/print');
	form.attr('target', '_blank');
	$('#btSubmit').val("print");
	$('#btSubmit').click();
	form.attr('action', base_url + 'report/expiry_no_renewal_report');
...
function ajax_filter(order_by='', order_type='') {
	...
	$.ajax({
		type: "POST",
		url: base_url + "report/expiry_no_renewal_report_rows/0",
		data: { 
...
function ajax_clear() {
	...
	ajax_clear_session('report', 'report_expiry_no_renewal_report_filter');
}
