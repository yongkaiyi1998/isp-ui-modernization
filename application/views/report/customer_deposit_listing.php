...
{
	var form = $('form');
	form.attr('action', base_url + 'report/export_customer_deposit_listing');
	$('#btSubmit').click();
	form.attr('action', base_url + 'report/customer_deposit_listing') ;
}
...
function ajax_filter(order_by='', order_type='') {
	...
	$.ajax({
		type: "POST",
		url: base_url + "report/customer_deposit_listing_rows/0",
		data: { 
...
function ajax_clear() {
		...
		<table class="table-striped" style='width:100%;'>
			<thead>
				...
				<tr class="report_th" style="background: #289383; font-weight: normal; color: #fff;">
					...
					<?php if ($isprint != 1) { ?><i class="menu-icon fa fa-sort" id="pay_date"></i><?php } ?>
					</th>
					...
					<?php if ($isprint != 1) { ?><i class="menu-icon fa fa-sort" id="building_name"></i><?php } ?>
					</th>
					...
					<?php if ($isprint != 1) { ?><i class="menu-icon fa fa-sort" id="type"></i><?php } ?>
					</th>
					<th class="text-center col-lg-1" <?php if ($isprint == 1) { echo 'style="width:10%;"'; } ?>>Amount
					<?php if ($isprint != 1) { ?><i class="menu-icon fa fa-sort" id="amount"></i><?php } ?>
					</th>
				</tr>
			</thead>
			<tbody>
	...
	ajax_clear_session('report', 'report_customer_deposit_listing_filter');
}
