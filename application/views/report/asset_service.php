...
<script>
	...
	{
		var form = $('form');
		form.attr('action', base_url + 'report/export_service_list');
		//form.attr('target', '_blank');
		$('#btSubmit').click();
		form.attr('action', base_url + 'report/service_list') ;
...
$( document ).on("click", "#btPrint", function(e){
	var form = $('form');
	form.attr('action', base_url + 'report/asset_service/print');
	form.attr('target', '_blank');
	$('#btSubmit').val("print");
	$('#btSubmit').click();
	form.attr('action', base_url + 'report/asset_service');
	$('#btSubmit').val("filter");
	form.removeAttr('target');
});
...
function ajax_filter() {
	...
	$.ajax({
		type: "POST",
		url: base_url + "report/asset_service_rows/0",
		data: { 
...
function ajax_clear() {
	...
	ajax_clear_session('report', 'report_asset_service_filter');
}
