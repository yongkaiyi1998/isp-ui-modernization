			...
			<div class="bg-success filter-bar hidden-print msg-print">
				...
				<form id="report" name="report" method="post" class="filter-form" action="<?php echo $form_action; ?>">
					...
					<div class="col-lg-12 bg-success">
						...
						<span class='filter-label'>GroupName </span>
						<input id="groupname" name="groupname" value="<?php echo $groupname; ?>" />
...
<script>
	...
	{
		var form = $('form');
		form.attr('action', base_url + 'report/export_radius_usergroup');
		//form.attr('target', '_blank');
		$('#btSubmit').click();
		form.attr('action', base_url + 'report/radius_usergroup') ;
...
$( document ).on("click", "#btPrint", function(e){
	var form = $('form');
	form.attr('action', base_url + 'report/radius_usergroup/print');
	form.attr('target', '_blank');
	$('#btSubmit').val("print");
	$('#btSubmit').click();
	form.attr('action', base_url + 'report/radius_usergroup');
...
function ajax_filter() {
	...
	$.ajax({
		type: "POST",
		url: base_url + "report/radius_usergroup_rows/0",
		data: { 
...
function ajax_clear() {
	...
	ajax_filter();
	ajax_clear_session('report', 'report_radius_usergroup_filter');
}
