			...
			<div class="bg-success filter-bar hidden-print msg-print">
				...
				<form id="report" name="report" method="post" class="filter-form" action="<?php echo $form_action; ?>">
					...
					<select id="sel_status" name="sel_status">
						...
						<?php
							foreach ($sel_statuslist as $val) {
								echo "<option value='" . $val . "' " . set_select('sel_status', $val, ($val==$sel_status ? true : false) ) . ">" . $val."</option> ";
							}
						?>
					</select>
...
{
	var form = $('form');
	form.attr('action', base_url + 'report/export_not_yet_customers');
	$('#btSubmit').click();
	form.attr('action', base_url + 'report/not_yet_customers') ;
}
...
function ajax_filter(order_by='', order_type='') {
	...
	$.ajax({
		type: "POST",
		url: base_url + "report/not_yet_customers_rows/0",
		data: { 
...
function ajax_clear() {
		...
		<table class="table-striped" style="width:100%;">
			<thead>
				...
				<tr class="report_th" style="background: #289383; font-weight: normal; color: #fff;">
					...
					<th class="col-lg-1">Company</th>
					<th class="col-lg-1">Phone</th>
					<th class="col-lg-1">Email</th>
					<th class="text-center col-lg-1">Status</th>
					<th class="text-left col-lg-2">Preferred Installation Date/Time</th>
				</tr>
			</thead>
			<tbody>
			</tbody>
	...
	ajax_clear_session('report', 'report_not_yet_customers_filter');
}
