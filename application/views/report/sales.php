		...
		<div class="panel-body">
			...
			<div class="bg-success filter-bar hidden-print msg-print">
				...
				<form id="report" name="report" method="post" class="filter-form" action="<?php echo $form_action; ?>">
					...
					<span> Date (First Activation): </span>
					...
					<button id="btPrint" name="btPrint" type="button" value="Print" class="btn btn-success" onclick="print_btn();">
...
<script src="<?php echo base_url("js/itelco/sales_report.js?".cssjs_ver()); ?>" ></script>
<script>
...
function print_btn()
{
	$(".main_info_table td").addClass('small');
	window.print();
	$(".main_info_table td").removeClass('small');
}
	
function generate_xls_filtered()
{
	var form = $('form');
	form.attr('action', base_url + 'report/export_sales');
	$('#btSubmit').click();
	form.attr('action', base_url + 'report/sales') ;
}
...
function ajax_filter(order_by='', order_type='') {
	...
	$.ajax({
		type: "POST",
		url: base_url + "report/sales_rows/0",
		data: { 
...
function ajax_clear() {
	...
	ajax_clear_session('report', 'report_sales_filter');
}
